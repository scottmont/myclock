...
def motor1():
	...
	if int(curlocation) > int(locationd):
		...
		screen.addstr(25, 4, "scott arm from %d to %d\n " % (oldlocation, curlocation))
		screen.addstr(26, 4,  "scott arm %d Microsteps BACKWARD to %d\n"  % (move, curlocation))
		...
		screen.addstr(25, 4, "scott arm from %s to %s\n" % (oldlocation, curlocation))
		screen.addstr(26, 4, "scott arm %s Microsteps FOREWARD to %s\n"  % (move, curlocation))
          	myStepper1.step( int(move), Adafruit_MotorHAT.BACKWARD,  Adafruit_MotorHAT.DOUBLE)
		turnOffMotors()
	else:
		screen.addstr(15, 4, "Scott current %f %f and stepper %d\n" % (latit,longt,curlocation))
	return
def motor2():
    while True:
	...
	global curlocation2
		...
		screen.addstr(27, 4, "juliana arm from %d to %d\n " % (oldlocation, curlocation2))
                screen.addstr(28, 4, "juliana arm %d Microsteps BACKWARD to %d\n"  % (move, curlocation2))
		...
		screen.addstr(27, 4, "juliana arm from %s to %s\n" % (oldlocation, curlocation2))
                screen.addstr(28, 4, "juliana arm %s Microsteps FOREWARD to %s\n"  % (move, curlocation2))
                myStepper2.step( int(move), Adafruit_MotorHAT.BACKWARD,  Adafruit_MotorHAT.DOUBLE)
                turnOffMotors()
        else:
		screen.addstr(16, 4, "juliana current %f %f and stepper %d\n" % (jlatit,jlongt,curlocation2))
	return
def motor3():
    while True:
        myStepper2 = mh.getStepper(200, 2)
	...
	try:
		...
		screen.addstr(28, 4, "maxim arm from %d to %d\n " % (oldlocation, curlocation3))
                screen.addstr(29, 4, "maxim arm %d Microsteps BACKWARD to %s\n"  % (move, curlocation3))
                myStepper2.step( int(move), Adafruit_MotorHAT.FORWARD, Adafruit_MotorHAT.DOUBLE)
                turnOffMotors()
		...
		screen.addstr(28, 4, "maxim arm from %s to %s\n" % (oldlocation, curlocation3))
                screen.addstr(29, 4, "maxim arm %s Microsteps FOREWARD to %s\n"  % (move, curlocation3))
                myStepper2.step( int(move), Adafruit_MotorHAT.BACKWARD,  Adafruit_MotorHAT.DOUBLE)
                turnOffMotors()
        else:
		screen.addstr(17, 4, "maxim current %f %f and stepper %d\n" % (mlatit,mlongt,curlocation3))
        return
def finder():
	while True:
		...
		global newlocation2
		global newlocation3
		screen.addstr(10, 4, "Clock has been started")
		screen.addstr(18, 4, time.strftime("%a, %d %b %Y %H:%M"))
		# Where is Scott?
		if 43.6358 <= float(latit) <= 43.6378 and -79.4185 >= float(longt) >= -79.4248 :
			...
			newlocation = 50
        		screen.addstr(15, 70, "scott at home" )
			motor1thread = threading.Thread(target=motor1)
                        motor1thread.start()
		else:
        		screen.addstr(15, 70, "dunno where scott is %f %f" % (latit, longt))
			newlocation = 200
			motor1thread = threading.Thread(target=motor1)
                        motor1thread.start()
		# Where is Juliana?
		...
		if 43.6909 <= float(mlatit) <= 43.6936 and -79.3315 >= float(mlongt) >= -79.3344 :
			newlocation3 = 100
                        screen.addstr(17, 70, "maxim at school" )
                        motor3thread = threading.Thread(target=motor3)
                        motor3thread.start()
		elif 43.6760 <= float(mlatit) <= 43.6769 and -79.3425 >= float(mlongt) >= -79.3439 :
			newlocation3 = 50 
                        screen.addstr(17, 70, "maxim at home" )
                        motor3thread = threading.Thread(target=motor3)
                        motor3thread.start()
		else:
			newlocation3 = 200
                        screen.addstr(17, 70, "dunno where maxim is %f %f\n" % (mlatit, mlongt))
                        motor3thread = threading.Thread(target=motor3)
                        motor3thread.start()
		screen.refresh()
		time.sleep(10)
...
while x != ord('4'):
     ...
     screen.addstr(5, 4, "2 - Quit")
     screen.addstr(6, 4, "3 - Reload")
